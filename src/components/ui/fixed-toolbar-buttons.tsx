...
import { KeyboardInputButton, SubscriptButton, SuperscriptButton } from './more-toolbar-button';
...
function getToolbarGroups(isStudentSite?: boolean, isSuggest?: boolean): ToolbarGroupDef[] {
    return [
        {
            id: 'insert',
            items: [
...
export function FixedToolbarButtons({ isStudentSite }: { isStudentSite?: boolean; }) {
    ...
    const groups = React.useMemo(
        () => getToolbarGroups(isStudentSite, isSuggesting),
    ...
    return (
        ...
        >
            <div
                ref={measureRef}
                className="fixed opacity-0 pointer-events-none flex -top-96 overflow-hidden"
            >
                {groups.map((group) => (
