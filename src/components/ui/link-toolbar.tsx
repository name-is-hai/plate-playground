...
export function LinkFloatingToolbar({
    ...
    const [target, setTarget] = React.useState<"_self" | "_blank" | "_parent" | "_top" | (string & {})>(attributes.target!);

    const isDisabled =
        !validateUrl(editState.editor, url ?? '') ||
        (url === attributes.href && target === attributes.target && text === attributes.text);
    ...
    const input = (
        <div className="flex w-[330px] flex-col" {...inputProps}>
            ...
            <div className="flex items-center gap-3">
                ...
                <label className="text-sm text-muted-foreground">Mở link bằng tab mới</label>
            </div>
            <Separator className="my-1" />
            <Button
                disabled={isDisabled}
